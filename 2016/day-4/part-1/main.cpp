...
auto main() -> int
{
    ...
    while (std::getline(input_file, line))
    {
        ...
        std::string sector_id = line.substr(line.find_last_of('-') + 1, line.find_last_of('[') - line.find_last_of('-') - 1);
        std::string checksum = line.substr(line.find_last_of('[') + 1, line.find_last_of(']') - line.find_last_of('[') - 1);
        ...
        std::vector<std::pair<char, int>> sorted_letters(letter_count.begin(), letter_count.end());
        std::sort(sorted_letters.begin(), sorted_letters.end(), [](const std::pair<char, int> &a, const std::pair<char, int> &b)
                  {
