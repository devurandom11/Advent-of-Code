...
var inputArr = fs
    ...
    .map(function (val, i, arr) { return val + arr[i + 1] + arr[i + 2]; });
var tracker = {
    ascending: 0,
    descending: 0,
};
inputArr.reduce(function (tracker, curr, index) {
    if (index === 0) {
        ...
        if (curr > inputArr[index - 1]) {
            tracker.ascending++;
            console.log("We are ascending from ".concat(inputArr[index - 1], " to ").concat(curr));
        }
        else if (curr < inputArr[index - 1]) {
            tracker.descending++;
            console.log("We are descending from ".concat(inputArr[index - 1], " to ").concat(curr));
        }
    }
    return tracker;
}, tracker);
